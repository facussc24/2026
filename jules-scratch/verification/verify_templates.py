...
def run_verification(playwright):
    ...
    try:
        ...
        # Verify the prompt was filled
        expect(page.locator("#ai-assistant-prompt-input")).to_have_value("Iniciar nuevo post para el blog sobre...")

        # Add a topic to the prompt
        page.locator("#ai-assistant-prompt-input").fill("Iniciar nuevo post para el blog sobre la nueva IA")
        ...
        expect(review_form.get_by_text("Investigar sobre la nueva IA")).to_be_visible()
        expect(review_form.get_by_text("Escribir primer borrador de la nueva IA")).to_be_visible()
        expect(review_form.get_by_text("Publicar post en el blog")).to_be_visible()

        page.screenshot(path="jules-scratch/verification/template_verification.png")
