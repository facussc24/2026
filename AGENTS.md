...
*   **Directive:** This system ensures users are always notified of new application versions. It is critical to follow this process precisely to avoid issues where updates are not detected due to caching or incorrect file generation.

#### Core Components
1.  **`scripts/generate-version.js`**: A Node.js script that reads the latest `git` commit hash and message.
2.  **`public/version.json`**: The output file generated by the script. This is the *only* file the frontend checks.
3.  **`public/js/version-checker.js`**: The frontend script that periodically fetches `version.json` to check for a new hash.
4.  **`firebase.json`**: The hosting configuration. It contains a **critical** caching rule for `version.json`.

#### Caching Configuration (CRITICAL)
*   The `firebase.json` file **must** contain the following header rule to prevent browsers and CDNs from caching `version.json`:
    ```json
    {
      "source": "version.json",
      "headers": [
    ...
    ```
*   **Reasoning:** Without this rule, Firebase Hosting may serve a stale version of the file for up to an hour by default, completely defeating the version check mechanism.

#### Correct Workflow for Releasing a New Version
1.  **Commit All Code Changes:** Ensure all your feature/bugfix code is committed to your working branch. The commit message for this code should be clear and descriptive for developers.
2.  **Create the Version Commit:** This is a separate, final commit.
    *   Run the command: `npm run version`. This will execute `scripts/generate-version.js`, which overwrites `public/version.json` with the hash and message of the *latest* commit.
    *   Commit **only** the changed `public/version.json` file. The message for *this* commit is what users will see in the "Release Notes".
    *   **CRITICAL:** This commit message must be user-centric.
        *   **GOOD:** `Feat: Se agrega la exportación de la lista de materiales a PDF.`
        *   **BAD:** `fix: apply patch`
3.  **Deploy:** Deploy the application to Firebase (e.g., `firebase deploy`).

*   **Summary:** The process creates a commit that "seals" a version. The `version.json` file points to this commit, and the frontend uses it to detect that a new version is available.
