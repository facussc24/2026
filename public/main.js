...
import { deleteProductAndOrphanedSubProducts } from './modules/products/product-logic.js';
...
import { renderControlPanelDashboard } from './modules/control_panel/control-panel.js';
import { renderEcrTableView } from './modules/control_panel/ecr-table-view.js';
import { renderIndicatorsView } from './modules/control_panel/indicators-view.js';
import { renderEcrSeguimientoView } from './modules/control_panel/ecr-seguimiento-view.js';
import { renderSeguimientoFichasView } from './modules/control_panel/seguimiento-fichas-view.js';
...
async function switchView(viewName, params = null) {
    ...
    else if (viewName === 'eco') {
        const deps = { db, firestore: { collection, query, orderBy, onSnapshot }, dom, lucide, switchView };
        const unsubscribe = await renderEcoListView(deps);
        appState.currentViewCleanup = unsubscribe;
    }
    else if (viewName === 'ecr') await runEcrLogic();
    else if (viewName === 'control_ecrs') {
        const deps = { dom, lucide, showInfoModal, appState, switchView, seedControlPanelTutorialData, newControlPanelTutorial };
        renderControlPanelDashboard(deps);
    }
    else if (viewName === 'seguimiento_ecr_eco') {
        const deps = { db, firestore: { collection, onSnapshot, query, addDoc, updateDoc, deleteDoc, doc, setDoc, getDoc, writeBatch, limit, orderBy }, dom, lucide, appState, showToast, switchView, showConfirmationModal, checkUserPermission };
        renderSeguimientoFichasView(deps);
    }
    else if (viewName === 'ecr_seguimiento') {
        const deps = { db, firestore: { collection, onSnapshot, query, addDoc, updateDoc, deleteDoc, doc, setDoc, getDoc }, dom, lucide, appState, showToast, switchView, showDatePromptModal };
        renderEcrSeguimientoView(deps);
    }
    else if (viewName === 'ecr_table_view') {
        const deps = { db, firestore: { collection, onSnapshot, getDoc, doc }, dom, lucide, appState, showToast };
        renderEcrTableView(deps);
    }
    else if (viewName === 'indicadores_ecm_view') {
        const deps = { db, firestore: { collection, onSnapshot, query, addDoc, updateDoc, deleteDoc, doc }, dom, lucide, appState, showToast, showConfirmationModal };
        renderIndicatorsView(deps);
    }
...
export const getFlattenedData = (product, activeFilters = {}) => {
    ...
    // BUG FIX (Verified 2025-09-12): When the level filter is an empty set, return an empty array.
    // This prevents the logic from incorrectly treating an empty filter as no filter.
